...
path = os.getcwd()
AUGMENTATION_DIR = path + '/utils/WBAugmenter_Python'


class WBEmulator:
    def __init__(self):
        self.features = np.load(AUGMENTATION_DIR + '/params/features.npy') # training encoded features
        self.mappingFuncs = np.load(AUGMENTATION_DIR + '/params/mappingFuncs.npy') # mapping functions to emulate WB effects
        self.encoderWeights = np.load(AUGMENTATION_DIR + '/params/encoderWeights.npy') # weight matrix for histogram encoding
        self.encoderBias = np.load(AUGMENTATION_DIR + '/params/encoderBias.npy') # bias vector for histogram encoding
    ...
    def single_image_processing(self, in_img, out_dir="../results", outNum=10, write_original=1):
        ...
        for i in range(outNum):  # save images
            ...
            image_list.append(out_dir + '/' + os.path.basename(filename) +
                        wb_pf[i] + file_extension)
            if write_original == 1:
                cv2.imwrite(out_dir + '/' + os.path.basename(filename) +
